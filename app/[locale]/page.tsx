...
import { getPosts, getServices, getApprovedReviews, getHeroVideoUrl } from "@/lib/wp";
...
export default async function HomePage({ params }: { params: { locale: string } }) {
  ...
  const [posts, services, reviews, heroVideoUrl] = await Promise.all([
    getPosts(),
    getServices(),
    getApprovedReviews(),
    getHeroVideoUrl(), // ✅ NEW
  ...
  const slides = [
    ...
    },
    {
      ...
      ctas: [
        {
          label: locale === "lg" ? "Laba ebyo bye tukola" : "See our services",
  ...
  return (
    <main>
      ...
      {/* HERO */}
      <HeroMotion slides={slides} locale={locale} heroVideoUrl={heroVideoUrl} />
      ...
      <TestimonialsSection
        ...
        items={reviewCards.map((r) => ({
          ...r,
          stars: "★★★★★☆☆☆☆☆".slice(
            5 - Math.max(0, Math.min(5, r.rating)),
            10 - Math.max(0, Math.min(5, r.rating))
          ),
        }))}
      />
